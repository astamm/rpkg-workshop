---
title: "R Packages"
subtitle: "⚔<br/>with usethis"
author: "Aymeric Stamm"
institute: "CNRS"
date: "2021/06/08 (updated: `r Sys.Date()`)"
output:
...
knitr::include_graphics('https://devtools.r-lib.org/reference/figures/logo.svg')
```
]
...
# Load [**devtools**](https://devtools.r-lib.org) and [**usethis**](https://usethis.r-lib.org) by default

https://usethis.r-lib.org/reference/rprofile-helper.html

These two functions open your `.Rprofile` and give you the code you need to paste in:

- `usethis::use_devtools()`: makes [**devtools**](https://devtools.r-lib.org) available in interactive sessions.
- `usethis::use_usethis()`: makes [**usethis**](https://usethis.r-lib.org) available in interactive sessions.

Your `.Rprofile` then looks like:
```{r eval=FALSE}
if (interactive()) {
  # Loads usethis and all other devtools packages
...
# Setting your Git credentials properly

- Log into your [GitHub](https://github.com/) account.
- Go to Settings > Developer Corner > Tokens, generate a Personal Access Token (PAT) and copy it to clipboard. See (https://docs.github.com/en/github/authenticating-to-github/creating-a-personal-access-token).
- In RStudio, run
  ```{r, eval=FALSE}
...
- Run
  ...
  ```
  to check that the new PAT is set up.

---

# Make RStudio remember your Git settings
...
options(
  usethis.full_name = "Aymeric Stamm",
...
- Adds important files to `.gitignore`.
- If user consents, makes an initial commit.

---

# Make your package a GitHub project
...
- Calls [`use_github_links()`](https://usethis.r-lib.org/reference/use_github_links.html), if the project is an R package.
- Configures `origin/DEFAULT` to be the upstream branch of the local `DEFAULT` branch, e.g. `master` or `main`.

---

# README - Your project's showcase

- Create a `README` file that briefly explains what your package does (it will serve as showcase on your GitHub project’s homepage).
  ```{r, eval=FALSE}
  usethis::use_readme_rmd()
  ```
  This function creates skeleton README files with sections for
  - a high-level description of the package and its goals.
  - R code to install from GitHub, if GitHub usage detected.
  - a basic example.

- Edit the `README.Rmd` file that just popped up by filling at least:
  - A section *Installation* explaining how users can install your software.
  - A section *Basic Usage* showing how to use the most important functions in your software.
- Knit it to obtain `README.md` using
  ```{r, eval=FALSE}
...
- Create a [codecov](https://about.codecov.io) account
- Login with your GitHub credentials
- Select your package in the proposed list
